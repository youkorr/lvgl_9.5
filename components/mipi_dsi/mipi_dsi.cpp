...
void MipiDsi::setup() {
  ...
  if (this->num_fbs_ >= 2) {
    esp_err_t fb_err;
    if (this->num_fbs_ >= 3) {
      fb_err = esp_lcd_dpi_panel_get_frame_buffer(this->handle_, 3, &this->framebuffers_[0],
                                                  &this->framebuffers_[1], &this->framebuffers_[2]);
    } else {
      fb_err = esp_lcd_dpi_panel_get_frame_buffer(this->handle_, 2, &this->framebuffers_[0],
                                                  &this->framebuffers_[1]);
    }
    if (fb_err != ESP_OK) {
      ESP_LOGW(TAG, "Could not get DPI framebuffers (%s); zero-copy present disabled",
               esp_err_to_name(fb_err));
      this->num_fbs_ = 1;
      this->framebuffers_[0] = this->framebuffers_[1] = this->framebuffers_[2] = nullptr;
    } else {
...
void MipiDsi::present_framebuffer(void *fb) {
  if (fb == nullptr || this->handle_ == nullptr)
    return;
  // Deferred wait: block for the PREVIOUS swap's transfer only now (not right
  ...
  // no pixel copy happens here (unlike write_to_display_'s draw_bitmap).
  esp_err_t err = esp_lcd_panel_draw_bitmap(this->handle_, 0, 0, this->width_, this->height_, fb);
  if (err != ESP_OK) {
    ESP_LOGE(TAG, "present_framebuffer draw_bitmap failed: %s", esp_err_to_name(err));
    return;
  }
