...
class MipiDsi final : public display::Display {
 public:
  ...
  uint8_t get_num_framebuffers() const { return this->num_fbs_; }
  // Pointer to DPI framebuffer @index (nullptr if unavailable). These are real
  // panel scan-out buffers: writing into one and presenting it swaps the panel
  // with zero copy.
  void *get_framebuffer(uint8_t index) const {
    return index < this->num_fbs_ ? this->framebuffers_[index] : nullptr;
  }
  size_t fb_width() const { return this->width_; }
  size_t fb_height() const { return this->height_; }
  // Present a framebuffer that was filled by the caller (e.g. LVGL rendered /
  // PPA-rotated directly into it). Because `fb` is a real DPI framebuffer,
  // esp_lcd_panel_draw_bitmap only swaps the scan-out pointer — no memcpy. The
  // wait for the previous transfer is deferred, so the CPU is not blocked here.
  void present_framebuffer(void *fb);
