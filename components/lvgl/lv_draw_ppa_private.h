...
static inline ppa_blend_color_mode_t lv_color_format_to_ppa_blend(lv_color_format_t lv_fmt)
{
    switch(lv_fmt) {
        ...
        case LV_COLOR_FORMAT_ARGB8888:
        /* XRGB8888 is 4 bytes/pixel like ARGB8888; only the alpha byte is
         * undefined. Falling through to the RGB565 default would make the PPA
         * read a 4 B/px buffer as 2 B/px and render garbage. Callers must force
         * the alpha (FIX_VALUE) for XRGB8888 since the X byte is meaningless. */
        case LV_COLOR_FORMAT_XRGB8888:
 ...
 * by that stride and not by its width. Describing a 100 px wide image stored
 * with a 256 byte stride as 100 px makes every row start 28 px early, which
 * shears the picture down the block.
 *
 * Returns 0 when the stride is not a whole number of pixels - RGB888 padded to
 * 320 bytes is 106.67 px - which the PPA cannot describe at all.
 */
static inline int32_t lv_ppa_pic_w(uint32_t stride, int32_t w, lv_color_format_t cf)
{
    if(stride == 0) return w;   /* LV_STRIDE_AUTO: the pitch is the width. */
